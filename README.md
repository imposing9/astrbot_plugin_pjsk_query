...
- 卡牌查询：支持卡牌名、卡牌 ID、角色 ID、角色名/角色英文名搜索，并附带卡牌图片
...
1. 将本目录放入 AstrBot 的 `data/plugins/astrbot_plugin_pjsk_query`。
2. 在 AstrBot 插件管理页重载插件，或重启 AstrBot。
3. AstrBot 会自动根据 `requirements.txt` 安装依赖 `aiohttp`。
